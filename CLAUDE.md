...
Marketing site for **Lúmina W S.A.S** (`luminaw.co`) — software-agency landing built on **Astro 6 + Tailwind v4 + TypeScript**, with React installed but currently unused in the rendered output. **Bilingual**: Spanish (Colombia, `es-CO`) is the default at `/`; English (`en`) is served under `/en/*`. Copy lives in `src/i18n/es.ts` + `src/i18n/en.ts` (every key must exist in both); locale/anchor helpers in `src/i18n/utils.ts`. Deploys to **Netlify** (`netlify.toml` + `public/_headers` + `public/_redirects`).

## Commands

```bash
npm run dev          # astro dev — http://localhost:4321 (no Netlify Functions)
npm run dev:netlify  # netlify dev — runs the contact Function (/api/contact) too
